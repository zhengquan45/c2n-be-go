...
type ProductContract struct {
	...
	Status                    uint      `json:"status" gorm:"type:int(4);default:1 not null"`
	...
	Decimals                  uint      `json:"decimals" gorm:"type:int(8);default:8 not null"`
	UnlockTime                time.Time `json:"unlock_time" gorm:"type:datetime"`
	Medias                    string    `json:"medias" gorm:"type:varchar(200)"`
	NumberOfRegistrations     uint      `json:"number_of_registrations" gorm:"type:int(8);default:0 not null"`
	...
	Type                      uint      `json:"type" gorm:"type:int(8);default:1 not null"`
	CardLink                  string    `json:"card_link" gorm:"type:varchar(200)"`
	TweetId                   string    `json:"tweet_id" gorm:"type:varchar(40)"`
	ChainId                   uint      `json:"chain_id" gorm:"type:int(8);default:0 not null"`
	PaymentTokenDecimals      uint      `json:"payment_token_decimals" gorm:"type:int(8);default:8 not null"`
	CurrentPrice              uint      `json:"current_price" gorm:"type:bigint(12);default:0 not null"`
}
